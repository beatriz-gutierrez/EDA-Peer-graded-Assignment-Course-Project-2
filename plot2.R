...
emission.plot <- with(baltimore.emissions.peryear,
                      barplot(height = baltimore.emissions.peryear$Emissions/1000, 
                              names.arg = baltimore.emissions.peryear$year,
                              col = 3: (length(yearsNEI)+2),
                              xlab = "Year",
                              ylab = expression('PM'[2.5]*' emission [Kilotons]'),
                              main = expression('Total PM'[2.5]*' emission in Baltimore city from all sources for each year'),
                              ylim = c(0, 4)
                      )
)

# add number of emissions at the top of each year-bar
emission.value <- round(baltimore.emissions.peryear$Emissions/1000, 2)
